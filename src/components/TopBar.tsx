...
export function TopBar({ sidebarCollapsed }: { sidebarCollapsed: boolean }) {
  ...
  return (
    <header className={`sticky top-0 z-30 h-12 bg-card border-b border-border flex items-center pl-14 pr-5 md:px-5 gap-4 transition-all duration-300 ease-out ${sidebarCollapsed ? "md:ml-[68px]" : "md:ml-[248px]"}`}>
      <form onSubmit={handleSearch} className="flex-1 w-full sm:max-w-xs lg:max-w-lg mx-auto">
        <div className="relative">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search loads..." aria-label="Search loads" className="w-full bg-muted border border-border rounded-lg pl-9 pr-4 py-1.5 text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-border" />
        </div>
      </form>

      <div className="flex items-center gap-2">
        <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="text-muted-foreground hover:text-foreground p-1.5 rounded-lg hover:bg-black/[0.03] dark:hover:bg-white/[0.03] transition-all hidden lg:flex items-center gap-1.5" aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}>
        ...
        {isLoggedIn ? (
          <>
            ...
            <div className="relative">
              <button onClick={() => setShowNotifs(!showNotifs)} className="relative text-muted-foreground hover:text-foreground p-1.5 rounded-lg hover:bg-black/[0.03] dark:hover:bg-white/[0.03] transition-all" aria-label={`Notifications${unreadCount > 0 ? ` (${unreadCount} unread)` : ""}`}>
                <Bell size={17} />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-primary flex items-center justify-center text-[9px] text-black font-bold">
              ...
              {showNotifs && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-80 bg-card border border-border rounded-lg overflow-hidden z-50 shadow-xl">
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.slice(0, 10).map(n => (
                          <button key={n.id} onClick={() => markRead.mutate(n.id)} className={`w-full text-left px-3 py-2.5 hover:bg-black/[0.03] dark:hover:bg-white/[0.03] transition-colors ${!n.read ? "bg-primary/10" : ""}`}>
                    ...
                    <div className="border-t border-border" />
                    <button onClick={() => { setShowNotifs(false); navigate("/settings"); }} className="w-full text-center py-2 text-[11px] text-primary hover:bg-black/[0.03] dark:hover:bg-white/[0.03] transition-colors">
            ...
            <div className="relative">
              <button className="flex items-center gap-2 hover:bg-black/[0.03] dark:hover:bg-white/[0.03] rounded-lg px-1.5 py-1 transition-all" onClick={() => setShowUserMenu(!showUserMenu)}>
                <div className="w-7 h-7 rounded-full bg-primary flex items-center justify-center font-display text-black text-[11px] font-bold">{userInitials}</div>
              ...
              {showUserMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-card border border-border rounded-lg overflow-hidden z-50 shadow-xl">
          ...
          <button onClick={() => openAuthModal("login")} className="flex items-center gap-2 bg-primary text-black font-display text-[12px] tracking-normal px-4 py-1.5 rounded-full hover:brightness-110 transition-all font-bold">
